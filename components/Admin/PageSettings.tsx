    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <h2 className="text-4xl font-extrabold text-slate-800 dark:text-white tracking-tight">Pages & Messaging</h2>
          <p className="text-slate-500 dark:text-slate-400 text-lg max-w-xl">Link your Meta assets and verify real-time connectivity.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {pages.length > 0 ? pages.map((page) => (
          <div key={page.id} className="bg-white dark:bg-slate-900 rounded-[48px] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col group hover:shadow-2xl hover:border-blue-100 dark:hover:border-blue-900 transition-all duration-500">
            <div className="p-10">
              <div className="flex items-center justify-between mb-8">
                <div className="w-14 h-14 bg-blue-50 dark:bg-blue-900/30 rounded-2xl flex items-center justify-center text-[#1877F2]">
                  <Facebook size={32} />
                </div>
                <button 
                  onClick={() => setAssigningPage(page)}
                  className="flex items-center gap-2 px-6 py-3 bg-slate-900 dark:bg-slate-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-lg active:scale-95"
                >
                  <UserPlus size={16} /> Manage Access
                </button>
              </div>
              <h3 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">{page.name}</h3>
                ...
                <div className="flex flex-wrap gap-3">
                  {safeArray(page.assignedAgentIds).length > 0 ? (
                    safeArray(page.assignedAgentIds).map(id => {
                      ...
                      return agent ? (
                        <div key={id} className="flex items-center gap-2 pl-1 pr-4 py-1.5 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-full text-[11px] font-bold border border-slate-100 dark:border-slate-700 group/item hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-bold text-amber-500 uppercase tracking-widest bg-amber-50 dark:bg-amber-900/20 px-4 py-2 rounded-2xl border border-amber-100 dark:border-amber-800">
            ...
            <div className="mt-auto p-6 bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center gap-4">
               <button 
                ...
                className={`flex-1 py-4 text-[11px] font-black uppercase tracking-widest rounded-2xl transition-all shadow-sm flex items-center justify-center gap-2 border ${
                   verificationResult?.id === page.id 
                    ...
                    : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:bg-slate-900 hover:text-white dark:hover:bg-blue-600'
                }`}
               >
               ...
               <button 
                ...
                className="p-4 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all border border-transparent hover:border-red-100 dark:hover:border-red-800"
          ...
          <div className="col-span-full py-20 bg-white dark:bg-slate-900 rounded-[48px] border-2 border-dashed border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center text-slate-300 dark:text-slate-600">
      ...
      {showImportModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[200] flex items-center justify-center p-6 animate-in fade-in">
           <div className="bg-white dark:bg-slate-900 rounded-[56px] shadow-2xl w-full max-w-lg p-12 animate-in zoom-in-95">
              <div className="flex justify-between items-center mb-8">
                 <h3 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Add Assets</h3>
                 <button onClick={() => setShowImportModal(false)} className="p-3 text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-full transition-colors"><X size={32} /></button>
              </div>
              <p className="text-slate-500 dark:text-slate-400 mb-10 text-lg font-medium leading-relaxed">Select the Facebook Pages you want to manage within the portal.</p>
                   ...
                   return (
                     <button 
                        ...
                        className={`w-full flex items-center justify-between p-6 rounded-[32px] border-2 transition-all group ${
                          isActive 
                            ? 'bg-blue-600 border-blue-600 text-white shadow-xl shadow-blue-200 dark:shadow-blue-900' 
                            : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-blue-400 text-slate-800 dark:text-slate-200'
                        }`}
                     >
                        <div className="flex items-center gap-4 text-left">
                           <div className={`p-3 rounded-2xl ${isActive ? 'bg-white/20' : 'bg-white dark:bg-slate-700'}`}>
              ...
              <button 
                ...
                className="w-full mt-10 py-6 bg-slate-900 dark:bg-blue-600 text-white rounded-[32px] font-black uppercase tracking-widest hover:bg-blue-600 dark:hover:bg-blue-700 transition-all shadow-2xl active:scale-95"
              >
                Sync Selected Pages
              </button>
           </div>
      ...
      {assigningPage && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white dark:bg-slate-900 rounded-[56px] shadow-[0_0_100px_rgba(0,0,0,0.2)] w-full max-w-md p-12 animate-in zoom-in-95 duration-300">
              <div className="flex justify-between items-center mb-4">
                 <h3 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Agent Permissions</h3>
                 <button onClick={() => setAssigningPage(null)} className="p-3 text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-full transition-colors"><X size={32} /></button>
              </div>
              <p className="text-slate-500 dark:text-slate-400 mb-10 text-lg">Who is authorized to chat for <span className="font-bold text-slate-900 dark:text-white underline decoration-blue-50 dark:decoration-blue-900">{assigningPage.name}</span>?</p>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center justify-between p-6 rounded-[32px] border-2 transition-all group ${
                      safeArray(assigningPage.assignedAgentIds).includes(agent.id)
                        ? 'bg-blue-600 border-blue-600 text-white shadow-xl shadow-blue-200 dark:shadow-blue-900' 
                        : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-blue-400 text-slate-800 dark:text-slate-200'
                    }`}
                  >
              ...
              <button 
                ...
                className="w-full mt-10 py-6 bg-slate-900 dark:bg-blue-600 text-white rounded-[32px] font-black uppercase tracking-widest hover:bg-blue-600 dark:hover:bg-blue-700 transition-all shadow-2xl active:scale-95 flex items-center justify-center gap-3"
              >
